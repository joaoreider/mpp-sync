...
def test_hrg04_custo_days_follow_baseline_dates(hrg04):
    from org.mpxj.reader import UniversalProjectReader

    from mpp_java import java_date, java_int, task_baseline_finish_at, task_baseline_start_at
    ...
    for task in project.getTasks():
        ...
        if uid is None:
            continue
        bounds[uid] = (
            java_date(task_baseline_start_at(task, 0)),
            java_date(task_baseline_finish_at(task, 0)),
        )
    outside = [
        (row.id_tarefa, row.hora_por_dia, bounds.get(row.id_tarefa))
        for row in hrg04.tarefas
        if row.custo
        and (
            row.hora_por_dia is None
            or bounds.get(row.id_tarefa, (None, None))[0] is None
            or bounds.get(row.id_tarefa, (None, None))[1] is None
            or row.hora_por_dia < bounds[row.id_tarefa][0]
            or row.hora_por_dia > bounds[row.id_tarefa][1]
        )
    ]
    assert outside == []
